...
bool readFileToMatrix(const std::string& filePath, Matrix& mat) {
  ...
  try {
    file >> mat; // Assuming the Matrix class has overloaded the >> operator to read from a file
  } catch (const std::exception& e) {
    std::cerr << "Error reading file into matrix: " << e.what() << std::endl;
    return false; // Failed to read data into the matrix
  }

  file.close(); // Close the file
  return true; // Successfully read data into the matrix
}
...
int check_matrix_read () {
  ...
  try {
    ...
    std::cerr << "bad, got an exception (supposed to succeed reading)"
              << std::endl;
    return 1;
  }
